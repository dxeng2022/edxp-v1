...
public class OrderDocCrossConverter {
    public List<CrossValidationVisualization> documentToVisualization(List<CrossValidationDocument> documents) {
    ...
    private List<SimilarSentence> createSimilarSentences(List<CrossValidationDocument> documents, List<Integer> ids, List<Double> similarities) {
        List<SimilarSentence> similarSentences = new ArrayList<>();

        for (int index = 0; index < ids.size(); index++) {
            int similarSentenceId = ids.get(index);
            CrossValidationDocument similarDocument = documents.stream()
