...
import {
  setVisualSHEETFile,
  setVisualSheetCloudAlert,
} from "../../actions/sheetAction";
...
const SheetCloudFile = () => {
  ...
  const visualSheetCloudAlert = useSelector(
    (state) => state.visualSheetCloudAlert
  );
  ...
  const rows = fileList
    .filter((file) => ["zip", "폴더"].includes(file.extension.toLowerCase()))
    .map((file, index) => ({
      id: index,
  ...
  const handleSelectCheckbox = (newSelectionModel, rows) => {
    ...
    dispatch(setVisualSHEETFile(filePath[0]));
  };
  ...
  return (
    <>
      ...
      <Dialog
        open={visualSheetCloudAlert}
        onClose={(event, reason) => {
          if (reason !== "backdropClick") {
            dispatch(setVisualSheetCloudAlert(false));
          }
        }}
      >
        <DialogTitle>시각화 오류</DialogTitle>
        <DialogContent>
          <DialogContentText>ipid 파일을 선택해주세요.</DialogContentText>
        </DialogContent>
        <DialogActions>
          <Button onClick={() => dispatch(setVisualSheetCloudAlert(false))}>
            닫기
          </Button>
        </DialogActions>
      </Dialog>
