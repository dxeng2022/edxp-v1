...
import DocVisualCrossAPI from '../services/DocVisualCrossAPI';
import CloudListAPI from '../services/CloudListAPI';


export default function VisualCrossCloudFile() {

  const dispatch = useDispatch();

  const { docVisualCrossCloudAPI } = DocVisualCrossAPI({});
  ...
  const rows = fileList
    .filter(file => ['csv', '폴더' ].includes(file.extension.toLowerCase()))
    .map((file, index) => ({
      id: index,
  ...
  return (
    <>
      ...
      <Box sx={{ py:1, pl: 2, display: 'flex', justifyContent: 'space-between', overflowX: 'auto' }}>
        ...
        <Button size="small" variant="contained" endIcon={<SendIcon />} sx={{ px:2, mr: 2, whiteSpace: 'nowrap' }} onClick={docVisualCrossCloudAPI}>
          시각화
        </Button>
      </Box>
