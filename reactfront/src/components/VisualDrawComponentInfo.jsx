...
export default function VisualDrawComponentInfo({ handleInfoClose, componentPath, componentName, getCount, matchedData, ratio, xy, page, rowsPerPage, handleCellClick, setPage }) {
  ...
  return (
    <Grid item component={Paper} elevation={12} xs={6} sm={6} md={6} sx={{ mt:'32px', pb:'10px', pt: 0, height: 'calc(100vh - 330px)',  borderRadius: '20px'  }} >
      ...
      <Box sx={{height: 'calc(100vh - 390px)', overflow:'auto'}}>
        <Box sx={{ pr:2, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', }}>
          <img
            src={'/proxy-image?imageKey=drawimg' + componentPath + componentName + '.png'}
            alt=""
            loading="lazy"
            style={{ maxWidth: '90%', height: '50%', border: '1px solid grey' }}
            />
