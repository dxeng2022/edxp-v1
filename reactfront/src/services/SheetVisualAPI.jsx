...
import {
  setVisualBackdrop,
  setVisualBackdropText,
  setVisualDrawCirclepackingData,
  setVisualDrawCloudAlert,
  setVisualDrawFile,
  setVisualDrawImg,
  setVisualDrawJson,
  setVisualDrawLineData,
  setVisualDrawSymbolData,
} from "../actions";
...
export default function VisualAPI() {
  ...
  const visualSheetLocalAPI = async (file) => {
    ...
    try {
      const jsonResponse = await axiosConfig.post(
        "/api/v1/sheet/result-loc",
        formData
      );
      ...
      if (jsonResponse.status === 200) {
        ...
        const circlePackingLineData = Object.entries(lines).map(
          ([name, loc]) => ({ name, loc })
        );
        const circlePackingSymbolData = Object.entries(symbols).map(
          ([name, loc]) => ({ name, loc })
        );
        ...
        const lineData = Object.entries(lines).map(([label, value]) => ({
          id: label,
          value,
          label,
        }));
        ...
        const symbolData = Object.entries(symbols).map(([label, value]) => ({
          id: label,
          value,
          label,
        }));
        ...
        try {
          const imgResponse = await axiosConfig.post(
            "/api/v1/sheet/result-img",
            visualDrawCloudInfo,
            {
              responseType: "blob",
            }
          ...
          if (imgResponse.status === 200) {
            ...
            const deleteResponse = await axiosConfig.delete(
              "/api/v1/sheet/result-delete"
            );
  ...
  const visualSheetCloudAPI = async () => {
  ...
  return { visualSheetLocalAPI, visualSheetCloudAPI };
}
